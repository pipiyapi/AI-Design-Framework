...
2. Run `scripts/query-memory.mjs` with the profile terms. Read only the returned Personal DNA, Project, Workflow, Pattern, Recipe, Principle, and Reference files. Use `references/relationships.json` to follow provenance and validation edges for selected results; do not load the entire knowledge folder.
3. Start with relevant Personal DNA and accepted Projects. Use a compatible Workflow for a demonstrated external sequence, a validated Pattern for a design rule, and a verified Recipe for implementation. A visual Reference is evidence, not a template; an accepted Project proves that related knowledge worked in practice.
...
- Do not reproduce every newly saved website. References and Patterns normally remain code-free. Accepted Projects own reproducible authored source snapshots; Recipes link to exact project files and extract only the reusable mechanism.
...
- Project feedback remains provisional until visually reviewed. After acceptance, connect it to the Accepted Project and rebuild the catalog and relationship graph.
